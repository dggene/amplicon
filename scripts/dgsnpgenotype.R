...
snp <- read.table(snpbed,head=FALSE)[,-2]#Ìæ»»allsnp.bed
     ...
     index=as.numeric(vcfindelhead[i,2])     
      for(j in 2:3){
               ...
               if(index>(pos-5)& index <(pos+5)){ 
                  if(genoindel[i,]=='0/0')  indelfinal[j,7]=indel[j,5]
                  if(genoindel[i,]=='0/1')  indelfinal[j,7]=indel[j,6]
                  if(genoindel[i,]=='1/1')  indelfinal[j,7]=indel[j,7]
                  indelfinal[j,6]=dp[i]
                  ...
                  indelfinal[j,5] = geno3[i,] 
                  indelfinal[j,4] = as.character(refindel[i,]) 
                   }
                  }  
           }            
   }  
